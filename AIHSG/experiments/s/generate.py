...
def generate_imp_hs(prompt,
    ...
    while step < max_length:
        ...
        full_names = []
        scores = []
        print(f"第{step}次logprobs的输出是：{logprobs}")
        for logprob in logprobs:
            if len(logprob) > 0 and isinstance(logprob[0], dict):
                ...
                full_names.extend("<pad>")  # 或者使用 None，或任何适当的占位符
                scores.append([0]*5)
        ...
        print("scores:",scores)
        # while len(scores) < 10:
        # scores = [sublist + [-100] * (5 - len(sublist)) if sublist else [0] * 5 for sublist in scores]
        print("score得输出：",scores)
        ...
        print("scores_tensor得形状",scores_tensor.shape)
        ...
        next_batch_beam = next_batch_beam[0]
    ...
    print("最后的输出")
    print("best_hypotheese:",best_hypotheses)
